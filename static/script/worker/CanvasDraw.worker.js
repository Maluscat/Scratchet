...
class CanvasDraw {
  ...
  // TODO user highlighting is not implemented yet
  redraw(posWrappers) {
    let posQueue = [];
    let lastCp;
    let prevPosData;
  ...
  getBezierControlPoints(startPos, computePos, endPos) {
    const distanceStartCompute = CanvasDraw.getPosDistance(startPos, computePos);
    const distanceComputeEnd = CanvasDraw.getPosDistance(computePos, endPos);
    const distanceTotal = distanceStartCompute + distanceComputeEnd;

    const scaleFactor1 = CanvasViewWorker.BEZIER_CONTROL_MOD * distanceStartCompute / distanceTotal;
    const scaleFactor2 = CanvasViewWorker.BEZIER_CONTROL_MOD * distanceComputeEnd / distanceTotal;
